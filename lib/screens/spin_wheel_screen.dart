...
class _SpinWheelScreenState extends State<SpinWheelScreen>
  ...
  static const String _lastSpinKey = 'spin_wheel_last_spin_date';
  ...
  Future<void> _checkDailySpin() async {
    ...
    final lastSpin = prefs.getString(_lastSpinKey);
  ...
  Future<void> _spin() async {
    ...
    final int selectedIndex = _random.nextInt(rewards.length);

    final int reward = rewards[selectedIndex];

    final double sectionAngle = (2 * pi) / rewards.length;
    ...
    final double targetSegment =
        (2 * pi) - (selectedIndex * sectionAngle) - (sectionAngle / 2);

    final double extraRotations = (5 + _random.nextInt(3)) * 2 * pi;

    final double targetRotation =
        _currentRotation + extraRotations + targetSegment;
    ...
    await prefs.setString(_lastSpinKey, _dateKey(DateTime.now()));
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          title: const Text('Wheel Stopped!', textAlign: TextAlign.center),

          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Your reward is', style: TextStyle(fontSize: 17)),
                    ...
                    child: Column(
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            onPressed: canSpin && !spinning ? _spin : null,
...
class _WheelPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < rewards.length; i++) {
      ...
      canvas.drawArc(rect, startAngle, sectionAngle, true, borderPaint);
    ...
    canvas.drawCircle(center, 34, Paint()..color = surfaceColor);

    canvas.drawCircle(center, 30, Paint()..color = primaryColor);
